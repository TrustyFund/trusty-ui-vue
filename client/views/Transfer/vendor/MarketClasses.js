...
function limitByPrecision(value, p = 8) {
  ...
  const splitString = valueString.split('.');
...
function didOrdersChange(newOrders, oldOrders) {
  ...
  newOrders.forEach((a, key) => {
    const oldOrder = oldOrders.get(key);
    if (!oldOrder) {
      changed = true;
    } else if (a.market_base === oldOrder.market_base) {
...
class Asset {
  constructor({
    asset_id = '1.3.0', amount = 0, precision = 5, real = null
  } = {}) {
...
class FeedPrice extends Price {
  constructor({
    priceObject, assets, market_base, sqr, real = false
  }) {
...
class LimitOrderCreate {
  constructor({
    for_sale, to_receive, seller = '', expiration = new Date(), fill_or_kill = false, fee = { amount: 0, asset_id: '1.3.0' }
  } = {}) {
...
class SettleOrder extends LimitOrder {
  ...
  amountToReceive() {
    const to_receive = this.for_sale.times(this.feed_price, this.isBid());
